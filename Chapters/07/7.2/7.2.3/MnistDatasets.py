...
def _partial_flatten(inputs):
    ...
    return jax.numpy.reshape(inputs, (inputs.reshape[0], -1))

def _one_hot(inputs, k, dtype = jax.numpy.float32):
...
def mnist(permute_train = False):
    ...
    train_images = _partial_flatten(train_labels) / jax.numpy.float32(255.)
    test_images = _partial_flatten(test_images) / jax.numpy.float32(255.)
