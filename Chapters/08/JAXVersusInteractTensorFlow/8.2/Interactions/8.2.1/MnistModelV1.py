...
def predict(inputs, weight):
    ...
    outputs = jax.numpy.dot(inputs, weight)
...
def train():
    ...
    # single_random_flattened_images = jax.random.normal(key, (28 * 28,))
    random_flattened_images = jax.random.normal(key, (10, 28 * 28))
    weight = jax.random.normal(key, (256, 784))

    # outputs = predict(random_flattened_images, weight)

    vmap_predict = jax.vmap(predict, [None, 0])

    outputs = vmap_predict(random_flattened_images, weight)
